...
> **BasedHardware Omi** 오픈소스 아키텍처를 계승하여 안드로이드 **Termux** 환경에서 일상 음성과 GPS를 Obsidian 마크다운 형식으로 상시 무한 기록하고, 위치 기반 **Chaos Logic(동적 Temperature)**과 Gemini API를 결합하여 이어폰으로 즉각 대답하는 독립형 AI 동반자 시스템.

---

## 🎯 Official Slogans & Philosophy

- **`soul scribe`** : 사용자의 일상과 영혼의 궤적을 빠짐없이 텍스트와 좌표로 영구 기록.
- **`self-hacker`** : 정체된 자아와 무의식적 습관을 상시 관찰하고 재정의하는 셀프 해킹 도구.
- **`alternative of gemini live`** : 무거운 앱 실행 없이 백그라운드에서 상시 대기하며 이어폰으로 즉각 피드백을 제공하는 Gemini Live의 강력한 온디바이스 대안.
- **`active cognitive hacking`** : 사용자의 공간 맥락(Chaos Geofencing)에 따라 정밀 추론과 확산적 영감을 주입하는 능동적 인지 해킹.

---

## 🌟 Omi 오픈소스 적극 활용 및 계승 구조

Soul Mate는 **[BasedHardware/omi](https://github.com/BasedHardware/omi)**의 3대 핵심 철학과 아키텍처를 Termux 환경에 맞추어 완벽히 구현했습니다:
...
1. **Omi Memory Schema & Lifelogging (`models.py`, `memory_manager.py`)**
   ...
   - Obsidian Vault(`~/storage/shared/Documents/SoulMate/YYYY-MM-DD.md`)에 실시간 타임스탬프 + GPS 메타데이터(`[Lat, Lon]`)를 무한 누적 기록.

2. **Omi Realtime Plugin Architecture (`plugins.py`)**
   - Omi의 실시간 오디오/텍스트 플러그인 이벤트 버스 설계.
   - `"소울메이트"`, `"메이트야"`, `"호들아"`, `"해킹해"`, `"오미야"`, `"헤이오미"` 등의 웨이크워드 감지 시 플러그인 자동 디스패치.
...
```text
soul-scribe/
├── config.py              # 전역 설정 (Gemini 모델, Geofence 거점, 웨이크워드, 슬로건)
